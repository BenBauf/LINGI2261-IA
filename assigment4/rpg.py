...
class Equipment:
    ...
    conflicts -- set of equipment names that conflict with this equipment
        ...
        with gzip.open(filename) as f:
            ...
            for line in f:
                ...
                if cmd == "Size":
                    ...
                    current_equ.conflicts = set(get_equ(e.strip())
                                                for e in args.split(", "))
...
class Level:
    ...
    def __init__(self, filename):
        ...
        def get_abi(name):
            if name not in self.abi_map:
                abi = Ability(len(self.abilities) + 1, name)
                self.abilities.append(abi)
                self.abi_map[name] = abi
            return self.abi_map[name]
    ...
    def __contains__(self, x):
        if isinstance(x, Ability):
            x = x.name
        return x in self.abi_map

    def __getitem__(self, x):
        if isinstance(x, int):
            return self.abilities[x - 1]
        else:
            return self.abi_map[x]
