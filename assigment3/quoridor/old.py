...
class MyAgent(Agent, minimax.Game):
    ...
    def getCurrentDepth(self,board,player):
        ...
        if(current < self.maxTime/2):
            ...
            self.count-=1
        if self.count>4 and self.depth<8:
            self.count=0
            self.depth+=1
            print(self.depth)
        if self.count<-4 and self.depth > 3:
            self.count=0
            self.depth-=1
    ...
    def cutoff(self, state, depth):
        ...
        """
        board, player = state
        return board.is_finished() or depth >= 5#self.depth
    ...
    def evaluate(self, state):
        ...
        if(costPlayer==0):
            print("costs : player ", costPlayer, " opponent ", costOpponent, " evaluate ", 10 + costOpponent-costPlayer )
            return 10 + costOpponent-costPlayer#+(board.nb_walls[player]/2)+(board.nb_walls[(player-1)%2]/2)
        else:
            print("costs : player ", costPlayer, " opponent ", costOpponent, " evaluate ", costOpponent-costPlayer )
            return costOpponent-costPlayer#+(board.nb_walls[player]/2)+(board.nb_walls[(player-1)%2]/2)
        #return score
